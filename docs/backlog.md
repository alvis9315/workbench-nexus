...
- [ ] **C5. NEON PROTOCOL 本命主題打樣**(§4):首角 **Vesper**,機能軍事／霓虹特勤世界觀,Low Top-Down。五組概念、八方向稿、六組 PixelLab 候選與三段動態立繪已歸檔至 `ui-asset-library/custom/sprites/vesper/`;目前缺可對應 idle／move／hover／grab／action 的透明連續幀,補齊後才建立可用主題。支線依序為動態立繪 → image-to-3D 偵察 → VRoid/VRM 正規 3D。
