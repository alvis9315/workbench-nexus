...
- Matter.js 建立 Engine + 地板／左右牆；頂部維持開放讓角色從畫面外落入，越界由 reset 保護
...
- 顯示策略:一般角色正規化到 72px；pose cell 較大的角色保留最多 2x 原生體型。每主題 roster 各自記憶；容量依 60–70／80–90／100–110／120–130／140–160% 分別為 30／24／18／14／10。放大不刪既有角色，只限制後續新增
...
- 2D 地面:水平中性灰格板；角色落到畫面下半部後，在不同 y 深度隨機選點，以正式 `move` 槽素材左右或斜向漫遊；移動間有隨機停頓、步速脈衝與小幅踏步起伏，停下切 idle，避免等速滑行。y 越大 z-index 越前；抓取時退出漫遊、恢復物理垂墜與 grab 素材，放開後再回到地面
- PRIZE OUT:出口位於左下。只有被使用者抓起後 1.8 秒內放進感應區才從 roster 移除，漫遊路過不會誤刪
