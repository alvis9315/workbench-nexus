...
**主路徑**:使用者貼 104 職缺連結 → 執行 agent 從 URL 抽 job ID → `GET http://localhost:<port>/jobs/{id}` 取完整明細(dashboard 後端須在跑)。

**fallback 鏈(非阻塞,依序降級)**:
1. 本機 API 沒開 → 直接抓 104 職缺頁解析
2. 抓不到(登入牆/改版)→ 請使用者貼職缺全文,照 2.2 schema 人工對映
3. 任何一層成功即繼續,失敗不中斷管線
...
- 2026-07-20 v0.1 初版。動機:三站原流程有兩個壞跳轉(職缺手動搬運、md 貼網頁版 Claude/GPT 排版),後者不可重現且個資外流,亦違反 criteria「輸出 .docx 優先」;收斂為連結貼回 + 本機渲染。
